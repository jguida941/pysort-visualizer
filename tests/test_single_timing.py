...
def test_single_slider_scrub_updates_logical_time(single_window, qtbot):  # noqa: F811
    names = sorted(INFO.keys())
    target = names[0]
    viz = single_window.visualizer_for(target)
    pane = single_window.pane_for(target)
    ...
    viz.prime_external_run([6, 3, 5, 4, 2])
    pane.reset()
    ...
    assert pane.step_index() == target_steps
    per_step = pane.logical_seconds() / target_steps
    assert per_step > 0.0

    viz.sld_scrub.setValue(0)
    qtbot.wait(1)
    assert pane.step_index() == 0
    assert pane.logical_seconds() == pytest.approx(0.0, abs=1e-6)

    viz.sld_scrub.setValue(2)
    qtbot.wait(1)
    assert pane.step_index() == 2
    assert pane.logical_seconds() == pytest.approx(per_step * 2, rel=0.05, abs=1e-6)
